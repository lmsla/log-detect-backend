...
| 功能                     | 目的                     | 關鍵檔案                                   |
| ---------------------- | ---------------------- | -------------------------------------- |
| **設備監控**               | 檢測線上/離線狀態，支援 HA 群組過濾   | detect.go, center.go                   |
| **YML 配置同步**           | 啟動時將 config.yml 同步至 DB | config_sync.go                         |
| **Elasticsearch 健康監控** | 監控 ES 叢集健康狀態           | es_monitor_service.go, es_scheduler.go |
| **電子郵件警報**             | 發送通知（含 HA 群組資訊）        | mail.go, detect.go                     |
...
Device (設備)
  ├─ DeviceGroup: 邏輯分組（對應 DeviceGroup.Name）
  ├─ Name: 設備主機名稱
  └─ HAGroup: HA 群組識別鍵（空字串 = 獨立裝置）
...
```
setting.yml            # 服務設定（資料庫連線、功能開關、郵件、CORS 等）
config.yml             # 擴充配置（ES 連線、監控目標 targets、裝置清單 devices）
devices.yml            # 裝置清單（選填，存在時覆蓋 config.yml 的 devices 區段）
```

### Feature Toggles 速查（setting.yml）

```yaml
features:
  timescaledb: true    # TimescaleDB 連線 + BatchWriter + 偵測 history 寫入
  es_monitoring: true  # ES 叢集健康監控排程
  dashboard: true      # 儀表板 API（依賴 timescaledb）
  auth: true           # JWT/RBAC 認證授權

config_source: "yml"   # "yml" = 啟動同步 config.yml → DB
                       # "api" = 透過前端/API 管理（預設）
```
  ...
  - device_group: "servers"
    names:
      - name: "server-01"
        ha_group: ""          # 空字串 = 獨立裝置
      - name: "fw-primary"
        ha_group: "fw-ha"     # 相同 ha_group = HA 叢集
      - name: "fw-standby"
        ha_group: "fw-ha"
```
