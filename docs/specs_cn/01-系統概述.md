...
- **裝置層級監控**：自動發現裝置、追蹤裝置狀態變化，支援 HA 群組感知告警
- **叢集層級監控**：Elasticsearch 健康狀態、效能指標
- **業務層級監控**：基於日誌模式的自定義監控

#### 2.1.2 雙模配置管理

- **API 模式**（`config_source: api`）：透過前端/REST API 管理配置，適合完整部署
- **YML 模式**（`config_source: yml`）：啟動時將 `config.yml` 同步至 MySQL，客戶只需編輯 YML 重啟服務即可，適合無前端精簡部署

#### 2.1.3 功能模組開關（Feature Toggles）

透過 `setting.yml` 的 `features` 區段按需啟用功能模組，支援最小化部署：

| 開關 | 控制範圍 |
|------|---------|
| `timescaledb` | TimescaleDB 連線、BatchWriter、偵測結果歷史寫入 |
| `es_monitoring` | ES 叢集健康監控排程 |
| `dashboard` | 儀表板 API |
| `auth` | JWT/RBAC 認證授權 |
...
log-detect-backend/
├── main.go                 # 應用程式進入點
├── config.yml             # 擴充配置（ES 連線、監控目標、裝置清單）
├── devices.yml            # 裝置清單（選填，存在時覆蓋 config.yml 的 devices 區段）
├── setting.yml            # 服務設定（資料庫、功能開關、郵件等）
│
├── clients/               # 外部服務客戶端
│   ├── mysql.go           # MySQL 連線管理
...
│   ├── user.go            # 使用者、角色、權限
│   ├── targets.go         # 監控目標、索引
│   ├── devices.go         # 裝置管理（含 ha_group 欄位）
│   ├── history.go         # 歷史記錄
│   └── elasticsearch.go   # ES 監控模型
│
├── services/              # 業務邏輯層
│   ├── auth.go            # 認證與授權
│   ├── config_sync.go     # YML → DB 同步服務（YML 模式啟動時執行）
│   ├── detect.go          # 核心監控邏輯（含 HA 群組過濾）
│   ├── center.go          # Cron 作業管理
│   ├── device.go          # 裝置管理
│   ├── device_group.go    # 裝置群組管理
│   ├── target.go          # 目標管理
│   ├── indices.go         # 索引管理
│   ├── history.go         # 歷史資料管理
│   ├── batch_writer.go    # 批次寫入服務（TimescaleDB）
│   ├── es_monitor.go      # ES 健康監控
│   ├── es_scheduler.go    # ES 監控排程器
│   ├── mail.go            # 郵件服務（含 HA 群組告警格式）
...
│
├── router/                # 路由配置
│   └── router.go          # API 路由定義（Feature Toggle 條件註冊）
│
├── models/                # 請求/回應模型
│   └── response.go        # 通用回應結構
│
├── global/                # 全域狀態
│   └── global.go          # 全域變數與配置（含 YMLConfig）
│
├── structs/               # 配置結構
│   ├── env.go             # 環境配置定義（含 Features、ConfigSource）
│   └── features.go        # Feature Toggles 與 YMLConfig struct 定義
│
├── utils/                 # 工具函數
│   └── utils.go           # 配置解析、CORS 等輔助功能
│
├── scripts/               # 輔助腳本
│   └── ha_simulator.go    # HA 群組測試資料產生器（寫入 Elasticsearch）
│
└── docs/                  # 文件
    ├── specs_cn/          # SDD 規格文件（本文件）
    ├── features/          # 功能設計文件
...
1. main.go 啟動
   ↓
2. 載入配置檔 (setting.yml + config.yml + devices.yml)
   ↓
3. 初始化 MySQL 連線（永遠執行）
   ↓
4. [if features.timescaledb] 初始化 TimescaleDB + BatchWriter
   ↓
5. 執行資料庫遷移 (RunMigrations)
   │ └─ TimescaleDB 遷移由 features.timescaledb 守衛
   ↓
6. [if config_source == "yml"] YML 同步至 DB (SyncConfigToDB)
   │ ├─ syncESConnectionsUpsert  — ES 連線 upsert
   │ ├─ syncDeviceGroups         — 裝置群組 upsert
   │ ├─ syncTargets + Indices    — 監控目標與索引 upsert + 刪除
   │ ├─ syncDevices              — 裝置清單 upsert + 刪除
   │ └─ syncESConnectionsDelete  — ES 連線軟刪除
   ↓
7. 初始化 Elasticsearch 客戶端 (SetElkClient)
   ↓
8. [if features.auth] 初始化認證系統
   │ ├─ 建立預設角色與權限
   │ └─ 建立預設管理員帳號
   ↓
9. 載入 Cron 排程器 (LoadCrontab)
   ↓
10. [if features.es_monitoring] 初始化 ES 監控排程器
    ↓
11. 啟動核心偵測排程 (Control_center)
    │ └─ 從 DB 載入所有啟用的監控目標並註冊 Cron 作業
    ↓
12. 載入路由與中介軟體（Auth 路由、Dashboard 等依 feature 條件註冊）
    ↓
13. 啟動 HTTP 服務器 (預設 :8006)
```
...
2. 執行 Detect() 函數
   ├─ 計算查詢時間範圍
   ├─ 查詢 Elasticsearch 取得有日誌的裝置清單
   ├─ 從資料庫取得已知裝置清單（依 device_group 過濾）
   └─ 比對兩份清單 → added / removed / intersection
   ↓
3. 處理新增裝置（auto-discovery）
   ├─ 若裝置已存在於任何群組 → 跳過（防止跨群組污染）
   └─ 否則 → 建立裝置記錄（含 YML ha_group 資訊）
   ↓
4. HA 群組過濾 (filterHAGroups)
   ├─ removed 裝置 → trulyRemoved（全組無人在線）或 standbyDevices（組內有人在線）
   └─ standbyDevices 不觸發告警
   ↓
5. 產生警報（trulyRemoved 不為空）
   └─ 發送電子郵件（含 HA 群組資訊）+ 寫入 mail_history
   ↓
6. [if features.timescaledb] 寫入偵測歷史
   └─ BatchWriter → TimescaleDB history 超表
...
| 版本  | 日期         | 變更內容            | 作者     |
| --- | ---------- | --------------- | ------ |
| 2.1 | 2026-03-30 | 新增 YML 模式、Feature Toggles、HA 群組、雙模配置說明；更新啟動流程與目錄結構 | Claude |
