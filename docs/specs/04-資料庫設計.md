...
| 5   | **device_groups**      | **裝置群組定義**   | < 100            |
| 6   | devices                | 監控裝置清單       | 10,000 - 100,000 |
| 7   | targets                | 監控目標         | < 100            |
| 8   | indices                | ES 索引配置      | < 100            |
| 9   | indices_targets        | 索引目標關聯       | < 500            |
| 10  | receivers              | 郵件接收者        | < 100            |
| 11  | history                | 監控歷史記錄       | 100,000+         |
| 12  | history_archive        | 歷史記錄封存       | 1,000,000+       |
| 13  | history_daily_stats    | 每日統計         | 10,000+          |
| 14  | mail_history           | 郵件發送記錄       | 10,000+          |
| 15  | alert_history          | 警報記錄         | 10,000+          |
| 16  | cron_lists             | Cron 作業清單    | < 500            |
| 17  | elasticsearch_monitors | ES 監控器配置     | < 50             |
...
CREATE TABLE `device_groups` (
  ...
  `deleted_at` INT,
  INDEX `idx_name` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='裝置群組表';
```

**欄位說明**：

- `name`: 群組識別鍵，對應 `devices.device_group`（透過名稱關聯，非外鍵）
- `deleted_at`: Unix timestamp 軟刪除，0 或 NULL 表示有效記錄
- 群組刪除限制：必須無裝置關聯才可刪除

**YML 同步行為**：

`config_source: yml` 模式下，`SyncConfigToDB()` 的 `syncDeviceGroups()` 會自動根據 `config.yml`（或 `devices.yml`）的 `devices` 區段建立對應群組記錄。
群組只做 upsert，**不自動刪除**（YML 移除的群組需透過 API 手動確認刪除）。
...
CREATE TABLE `devices` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `device_group` VARCHAR(100) NOT NULL COMMENT '裝置群組（對應 device_groups.name）',
  `name` VARCHAR(255) NOT NULL COMMENT '裝置名稱',
  `ha_group` VARCHAR(50) DEFAULT '' COMMENT 'HA 群組識別鍵，空字串表示獨立裝置',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
- `device_group`: 群組名稱，與 `device_groups.name` 透過字串關聯（非外鍵約束）
- `ha_group`: HA 叢集識別鍵。相同 `ha_group` 的裝置視為同一 HA 叢集，只有全組失聯才觸發告警；空字串表示獨立裝置，失聯即告警
...
```
ha_group = ""    → 獨立裝置，任何時間沒有日誌即觸發告警
ha_group = "X"  → HA 裝置，需同組所有裝置都沒有日誌才觸發告警
                  同組有任一裝置在線 → 其餘記錄為 standby（不告警）
```
...
#### 2.3.11 elasticsearch_monitors（ES 監控器）
...
本章節記錄計畫中但尚未實作的資料庫擴充功能，用於指導未來開發工作。
...
#### 10.3.1 ES 連線 - TLS 憑證驗證支援

> **優先級**：🟢 低
> **追蹤問題**：Issue #32
> **預計版本**：v2.0
> **關聯文件**：[05-安全與部署.md - TLS/SSL 安全](#43-tlsssl-安全)

##### 背景說明

當前系統使用 `InsecureSkipVerify: true` 跳過 TLS 憑證驗證，適用於開發環境和自簽憑證場景。正式環境需要完整憑證驗證以確保連線安全。

**當前實作**：

```yaml
# es_connections 表（現有欄位）
- use_tls: TINYINT(1)       # 決定使用 http 或 https
- enable_auth: TINYINT(1)   # 是否啟用認證
- username: VARCHAR(100)    # 認證使用者
- password: VARCHAR(255)    # 認證密碼
```

**運作邏輯**：

- `use_tls = true` → 使用 `https://` 協議
- `use_tls = false` → 使用 `http://` 協議
- `InsecureSkipVerify = true` → 跳過憑證驗證（目前寫死在程式碼中）
...
| 欄位名稱                 | 類型           | 預設值  | NULL     | 說明                       |
| -------------------- | ------------ | ---- | -------- | ------------------------ |
| `verify_certificate` | TINYINT(1)   | 0    | NOT NULL | 是否驗證 TLS 憑證（0=跳過驗證，1=驗證） |
| `ca_cert_path`       | VARCHAR(500) | NULL | NULL     | CA 憑證檔案路徑（用於驗證伺服器憑證）     |
| `client_cert_path`   | VARCHAR(500) | NULL | NULL     | 客戶端憑證路徑（mTLS，選配）         |
| `client_key_path`    | VARCHAR(500) | NULL | NULL     | 客戶端私鑰路徑（mTLS，選配）         |
...
CREATE TABLE `es_connections` (
  ...
  `username` VARCHAR(100),
  `password` VARCHAR(255),
  `enable_auth` TINYINT(1) DEFAULT 0 COMMENT '是否啟用帳號密碼認證',
  `use_tls` TINYINT(1) DEFAULT 1 COMMENT '是否使用 HTTPS 協議',

  -- 新增欄位
  `verify_certificate` TINYINT(1) DEFAULT 0 COMMENT '是否驗證 TLS 憑證',
  `ca_cert_path` VARCHAR(500) COMMENT 'CA 憑證檔案路徑',
  `client_cert_path` VARCHAR(500) COMMENT '客戶端憑證路徑（mTLS）',
  `client_key_path` VARCHAR(500) COMMENT '客戶端私鑰路徑（mTLS）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Elasticsearch 連線配置';
```

##### Migration 腳本

**Up Migration**（`xxx_add_tls_verification.up.sql`）：

```sql
-- 新增 TLS 憑證驗證支援欄位
ALTER TABLE es_connections
ADD COLUMN verify_certificate TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否驗證 TLS 憑證' AFTER use_tls,
ADD COLUMN ca_cert_path VARCHAR(500) DEFAULT NULL COMMENT 'CA 憑證檔案路徑' AFTER verify_certificate,
ADD COLUMN client_cert_path VARCHAR(500) DEFAULT NULL COMMENT '客戶端憑證路徑（mTLS）' AFTER ca_cert_path,
ADD COLUMN client_key_path VARCHAR(500) DEFAULT NULL COMMENT '客戶端私鑰路徑（mTLS）' AFTER client_cert_path;

-- 更新註解
ALTER TABLE es_connections COMMENT = 'Elasticsearch 連線配置（含 TLS 憑證驗證）';
```

**Down Migration**（`xxx_add_tls_verification.down.sql`）：
...
DROP COLUMN verify_certificate;

-- 恢復原註解
ALTER TABLE es_connections COMMENT = 'Elasticsearch 連線配置';
```

##### Entity 結構擴充

**Go 結構定義**（`entities/es_connection.go`）：
...
type ESConnection struct {
    ...
    Port        int    `gorm:"type:int;not null;default:9200" json:"port"`
    Username    string `gorm:"type:varchar(100)" json:"username"`
    Password    string `gorm:"type:varchar(255)" json:"password,omitempty"`
    EnableAuth  bool   `gorm:"type:tinyint(1);default:0" json:"enable_auth"`
    UseTLS      bool   `gorm:"type:tinyint(1);default:1" json:"use_tls"`

    // TLS 憑證驗證（新增欄位）
    VerifyCertificate bool   `gorm:"type:tinyint(1);default:0" json:"verify_certificate"`
    CACertPath        string `gorm:"type:varchar(500)" json:"ca_cert_path,omitempty"`
    ClientCertPath    string `gorm:"type:varchar(500)" json:"client_cert_path,omitempty"`
    ClientKeyPath     string `gorm:"type:varchar(500)" json:"client_key_path,omitempty"`
...
- `verify_certificate` 預設為 0（跳過驗證）
- 現有連線配置無需修改
- 程式碼邏輯：`InsecureSkipVerify = !conn.VerifyCertificate`

**2. 憑證路徑管理**

支援以下格式：

- **絕對路徑**：`/etc/ssl/certs/es-ca.crt`
- **相對路徑**：`./certs/ca.crt`（相對於應用程式執行目錄）
- **環境變數**：可在前端或後端處理
...
# 客戶端憑證和私鑰（僅應用程式可讀）
-rw-------  logdetect logdetect  client.crt
-rw-------  logdetect logdetect  client.key
```
...
```sql
INSERT INTO es_connections (name, host, port, use_tls, verify_certificate)
VALUES ('Dev ES', '10.99.1.213', 9200, 1, 0);
```

結果：使用 `https://10.99.1.213:9200`，跳過憑證驗證 ✅

**情境 2：正式環境（標準 TLS）**

```sql
INSERT INTO es_connections (
    name, host, port, use_tls, verify_certificate, ca_cert_path
) VALUES (
    'Prod ES', 'es-cluster.example.com', 9200, 1, 1, '/etc/ssl/certs/es-ca.crt'
);
```

結果：使用 `https://es-cluster.example.com:9200`，驗證憑證 ✅

**情境 3：正式環境（雙向 TLS）**

```sql
INSERT INTO es_connections (
    name, host, port, use_tls, verify_certificate,
    ca_cert_path, client_cert_path, client_key_path
) VALUES (
    'Prod ES mTLS', 'secure-es.example.com', 9200, 1, 1,
    '/etc/ssl/certs/es-ca.crt',
    '/etc/ssl/certs/client.crt',
    '/etc/ssl/private/client.key'
);
```

結果：使用 `https://secure-es.example.com:9200`，雙向身份驗證 ✅

##### 前端整合

詳見：[05-安全與部署.md - 4.3.2 未來增強（正式環境規劃）](#432-未來增強正式環境規劃)
...
- **記憶體**：憑證載入增加約 10-50KB（每個連線）
- **CPU**：憑證驗證增加約 1-5ms 延遲（首次連線）
- **磁碟**：憑證檔案約 1-5KB
...
1. **Phase 1**（v1.5）：資料庫擴充和 Migration
2. **Phase 2**（v1.6）：後端支援和憑證載入邏輯
3. **Phase 3**（v1.7）：前端 UI 和使用者體驗
4. **Phase 4**（v2.0）：文件、測試和安全審查
