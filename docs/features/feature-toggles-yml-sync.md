...
| 精簡部署（無前端） | `yml` | 按需啟用 | 客戶只接觸 YML；`config.yml` / `setting.yml` 需重啟，`devices.yml` 可選擇 hot reload |
...
features:
  ...
  es_monitoring: false     # ES 叢集健康監控排程（執行期查詢與排程仍需 TimescaleDB 狀態正常）
...
### 5. 自動發現同群組去重 (`services/detect.go`)

系統在自動發現邏輯中會檢查 **同一個 `device_group` 內** 是否已存在相同 `name`。
若同群組已存在則跳過建立；若僅存在於其他群組，仍允許建立，因此同名設備可存在於不同群組。

```go
// 自動發現前先確認設備不存在於同一群組
var existCount int64
global.Mysql.Model(&entities.Device{}).
    Where("device_group = ? AND name = ?", deviceGroup, device).
    Count(&existCount)
if existCount > 0 {
    // 跳過，該裝置已存在於同一個 device_group
    continue
}
```
